...
std::vector<int> bubbleSortOnce(const std::vector<int>& input)
{
    ...
    for (int i = 0; i < result.size() - 1; ++i)if (result[i] > result[i + 1])std::swap(result[i], result[i + 1]);
    return result;
}
int main()
{
    ...
    std::cout << (bubbleSortOnce({ 9, 7, 5, 3, 1, 2, 4, 6, 8 }) == std::vector<int>{ 7, 5, 3, 1, 2, 4, 6, 8, 9 }) << "\n";
