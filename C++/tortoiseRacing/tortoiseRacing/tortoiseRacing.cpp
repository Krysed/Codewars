...
public:
    static std::vector<int> race(int v1, int v2, int g) {
        ...
        int time = g * 3600 / (v1 - v2);
        int hour = time / 3600;
        int minute = (time / 60) % 60;
        int second = time % 60;
        return { abs(hour),abs(minute),abs(second) };
    }
};
...
int main()
{
    ...
    std::cout << equal(Tortoise::race(720, 850, 70), { 0,32,18 }) << "\n";
    std::cout << equal(Tortoise::race(80, 91, 37), { 3,21,49 }) << "\n";
